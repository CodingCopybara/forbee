...
def process_analysis_in_background(request: ImageAnalysisRequest):
    ...
    try:
        ...
        logging.info("\n    - Downloading image from: %s", request.imageUrl)
        ...
        if USE_AZURE_STORAGE:
            ...
            if not is_success: raise Exception("Failed to encode result image to JPEG format.")
            ...
            logging.info("\n    - Result image uploaded to Azure Blob Storage: %s", result_relative_path)
            ...
            logging.info("\n    - Result image saved locally to: %s", full_save_path)
        ...
        response_data = ImageAnalysisResult(
            ...
            resultImagePath=result_url, # 생성된 URL (로컬 또는 Azure)
        ...
        logging.info("\n    - Analysis result for user '%s' published to Kafka topic '%s'.", request.userId, kafka_topic)
...
async def analyze_image(request: ImageAnalysisRequest, background_tasks: BackgroundTasks):
    """
    이미지 분석 요청을 즉시 수락하고 200 OK 응답 반환 후 백그라운드에서 처리
    """
