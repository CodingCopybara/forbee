...
class TorchDatasetWrapper:
    ...
    def cast_sample_to_tensors(sample: dict,
        ...
        new_sample = {
            ...
            'year': sample['year'],
            'bloom_doy': torch.tensor(sample['bloom_doy']).to(device) if 'bloom_doy' in sample else torch.tensor(-1).to(device), # Use -1 as a placeholder if not present
            'bloom_ix': torch.tensor(sample['bloom_ix']).to(device) if 'bloom_ix' in sample else torch.tensor(-1).to(device), # Use -1 as a placeholder if not present,
            'bloom_date': sample['bloom_date'] if 'bloom_date' in sample else None,
            'original': sample,
        }
