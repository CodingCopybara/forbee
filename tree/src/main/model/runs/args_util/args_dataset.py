...
KOREAN_LOCATION_GROUP_TO_SPECIES = {
    'korean_forsythia': '개나리',
    'korean_plum_blossom': '매화',
    'korean_cherry_blossom': '벚나무',
    'korean_acacia': '아까시나무',
    'korean_azalea': '진달래',
    'korean_cosmos': '코스모스',
}
...
def configure_argparser_dataset(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument('--temperature_src',
                        type=str,
                        choices=config.SOURCES_TEMPERATURE,
                        default=config.SOURCES_TEMPERATURE[0],
                        help='Source of temperature data',
                        )
    parser.add_argument('--temperature_csv_path',
                        type=str,
                        help='Path to the temperature csv file, required when temperature_src is user_data',
                        )
...
def get_configured_dataset(args: argparse.Namespace) -> tuple:
    ...
    if args.temperature_src == 'user_data':
        assert args.temperature_csv_path is not None, "temperature_csv_path must be specified when using user_data"
    ...
    # Determine target_species if a Korean flower group is selected
    target_species = None
    if args.locations in KOREAN_LOCATION_GROUP_TO_SPECIES:
        target_species = KOREAN_LOCATION_GROUP_TO_SPECIES[args.locations]
    ...
    kwargs = {
        ...
        'temperature_csv_path': args.temperature_csv_path,
