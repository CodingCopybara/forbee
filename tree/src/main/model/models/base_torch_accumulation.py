...
class BaseTorchAccumulationModel(BaseTorchModel):
    ...
    SCALE_CHILL = config.SEASON_LENGTH
    SCALE_GROWTH = config.SEASON_LENGTH
    ...
    def forward(self,
        ...
        ix = ix.clamp(min=0, max=config.SEASON_LENGTH - 1)
        ...
        return ix, {
            ...
            'ix_chill': (1 - (units_c_cs >= th_c).to(torch.int)).sum(dim=-1).clamp(min=0, max=config.SEASON_LENGTH - 1),
    ...
    def _bce_loss(self, xs: dict, scale: float = 1e-3) -> tuple:
        ...
        req_g_true = torch.cat(
            [torch.arange(config.SEASON_LENGTH).unsqueeze(0) for _ in range(bs)], dim=0
    ...
    def _nll_loss(self, xs: dict, scale: float = 1) -> tuple:
        ...
        # Handle ys_true - 1 for the first day (index 0)
        # If ys_true is 0, then prev_req_g_pred should be 0
        prev_req_g_pred = torch.where(ys_true == 0, 
                                      torch.zeros_like(req_g_pred[bixs, ys_true]), 
                                      req_g_pred[bixs, ys_true - 1])

        ps = req_g_pred[bixs, ys_true] - prev_req_g_pred + epsilon
        ps = torch.clamp(ps, min=epsilon, max=1.0 - epsilon)
